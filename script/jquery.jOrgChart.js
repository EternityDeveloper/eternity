...
(function($) {
  ...
  function buildNode($node, $appendTo, level, opts) {
	...
	//if ($($node).attr("width")>0) alert($($node).attr("width"));
...
//$nodeDiv
//	$($nodeDiv).width($($node).attr("width"));
	if ($($node).attr("width")>0) $($nodeDiv).width($($node).attr("width"));
	if ($($node).attr("class")!=null) $($nodeDiv).addClass($($node).attr("class"));
