...
class Prospectos{
		...
		if ($proceder){
			...
			if ($this->prospectExist($person_id)>0){ 
				...
				$comercial_correlativo=mysql_insert_id($this->db_link->link_id);
                                ...
                                $track->fecha_inicio_cliente="curdate()";
                                $track->fecha_proxima="curdate()";
                                $track->id_actividad="INI";
                                $track->detalle_actividad="INICIO DE PROSPECTACION";
                                $track->hora="00:00:00";
                                $track->lugar="";
                                $track->apoyo=0;
                                ...
                                //$kk=array("mensaje"=>"error al grabar".mysql_error()."query".$SQL,"error"=>true);;
                                //return $kk;
	...
	public function doCreateReportCache($pilar,$id_comercial,$person_id,$comercial_correlativo,$pilar_final="",$actividad=""){
...
WHERE 
	prospecto_comercial.correlativo='".$comercial_correlativo."'";
		...
		while($row=mysql_fetch_assoc($rs)){
                      ...
                      /*  $prueba2= new ObjectSQL();
                        $prueba2->texto="entro al while".$query;
                        $SQL2=$prueba2->getSQL("insert","prueba");
                        mysql_query($SQL2); */ 
			...
			if ($clasificacion!=""){
 			...
 			$SQL3="SELECT COUNT(*) AS total FROM cache_listado_prospectos 
					WHERE  id_nit='".$person_id."' and correlativo='".$comercial_correlativo."'";
  			$rs=mysql_query($SQL3);
			$row=mysql_fetch_assoc($rs);
                        
                        if (!$rs) {
                                     $paso = array("mensaje"=>"si pasa por aqui".mysql_error()."query".$SQL,"error"=>true);
                                    return $paso;
                           }  
			if ($row['total']==0){

                                if ($ob->fecha_ultima_actividad = ' ') {
                                    $ob->fecha_ultima_actividad = "00-00-0000 00:00:00";
                                }


				$SQL=$ob->toSQL("insert");
                               /* if (!$SQL) {
                                    $retur = array("mensaje"=>"error al generar query","error"=>true);
                                    }*/


                                
                             //   $SQL4=$ob->getSQL("insert","cache_listado_prospectos"); 	
                             /*   $prueba2= new ObjectSQL();
                                $prueba2->texto=$SQL4;
                                $SQL2=$prueba2->getSQL("insert","prueba");
                                mysql_query($SQL2); */ 
			...
			//	$SQL=$ob->toSQL("insert");
                        //        $SQL=$ob->getSQL("insert","cache_listado_prospectos"); 	
				$prueba = mysql_query($SQL);
                                if (!$prueba) {
                                   $retur = array("mensaje"=>"error grabar".mysql_error()."query".$SQL,"error"=>true);
                                   return $retur;
                                 } 
			}else{
                              /*  $prueba2= new ObjectSQL();
                                $prueba2->texto="paso al update";
                                $SQL2=$prueba2->getSQL("insert","prueba");
                                mysql_query($SQL2); */
