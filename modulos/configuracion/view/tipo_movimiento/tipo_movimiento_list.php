...
if (!isset($protect)){
	...
	if (isset($_REQUEST['view_edit_movimiento'])){
		include("view/edit.php");
		exit; 
	}
 	/*VALIDA SI UN DOCUMENTO EXISTE*/
	if (isset($_REQUEST['validateMovExist'])){
		$data=array("error"=>true,"mensaje"=>'La información proporcionada no esta completa!');
		if (validateField($_REQUEST,"TIPO_MOV")){
		SystemHtml::getInstance()->includeClass("configuracion","CTipoMovimiento"); 
			$man= new CTipoMovimiento($protect->getDBLink()); 
			$result=$man->validateMovExist($_REQUEST['TIPO_MOV']); 
			if ($result==0){
				$data['exist']=false;
				$data['error']=false;
				$data['mensaje']='';
	...
	if (isset($_REQUEST['save_new_mov'])){
		
		$data=array("error"=>true,"mensaje"=>'La información proporcionada no esta completa!');
		/*
			VALIDO TODA LA INFORMACION
		*/
		if ((validateField($_REQUEST,"TIPO_MOV") && validateField($_REQUEST,"DESCRIPCION") )){
	...
	if (isset($_REQUEST['save_edit_mov'])){
		$data=array("error"=>true,"mensaje"=>'La información proporcionada no esta completa!');
		/* VALIDO TODA LA INFORMACION */
		if ((validateField($_REQUEST,"TIPO_MOV") && validateField($_REQUEST,"DESCRIPCION") )){
			SystemHtml::getInstance()->includeClass("configuracion","CTipoMovimiento"); 
	...
	SystemHtml::getInstance()->addTagScriptByModule("Class.TipoMovimiento.js"); 
...
$(function(){ 						
  	_man= new TipoMovimiento('content_dialog');
...
<div id="man_" class="fsPage">
  <h2>Mantenimiento de Tipo Movimientos</h2>
  <button id="create_caja" class="greenButton">Agregar tipo movimiento</button>
 	<table border="0" class="display" id="v_list" style="font-size:13px">
      <thead>
        <tr>
          ...
          <th>Descripcion</th>
          <th>Interno</th>
          <th>Cueta Contable</th>
          <th>CASH</th>
          <th>Autorizacin</th>
          <th>Afecta Contrato</th>
          <th>Afecta Cliente</th>
          <th>Afecta Reserva</th>
          <th>Operacion</th>
          <th>Afecta Cuotas</th>
          <th>Afecta Mora</th>
          <th>Afecta Mantenimiento</th>
          <th>&nbsp;</th>
        </tr>
      </thead>
      <tbody>
