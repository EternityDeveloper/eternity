...
/*NOTA DE DEBITO*/
if (isset($_REQUEST['CReqPagoMantenimiento'])){  
	if (validateField($_REQUEST,"contrato")&& validateField($_REQUEST,"monto_abono")){ 
		...
		if($monto_abono>0){
			...
			$docto=MVFactura::getInstance()->doCreateReciboRequerimiento($cdata->id_nit_cliente,
																		...
																		1,
																		date("Y-m-d"),
																		"GENERANDO RECIBO PARA COBRO POR VENTANILLA DE MANTENIMIENTO",
																		$tasa,
																		0,
																		0,
																		0
...
/*vista para agregar tipo de cobro tarjeta de credito*/
if (isset($_REQUEST['view_metodo_cobro_credit_card'])){  
	include("contrato/view/view_metodo_cobro.php"); 
	exit;
} 
if (isset($_REQUEST['doSaveMetodoCobroTC'])){  
	if (validateField($_REQUEST,"contrato") && validateField($_REQUEST,"metodo_cobro")
	 && validateField($_REQUEST,"tipo_tarjeta")  && validateField($_REQUEST,"numero_tarjeta")
	  && validateField($_REQUEST,"cvv_metodo")&& validateField($_REQUEST,"month_venc")
	  && validateField($_REQUEST,"year_venc") && validateField($_REQUEST,"dias_debito")){ 
	 	SystemHtml::getInstance()->includeClass("contratos","Contratos"); 	 
		$ct = json_decode(System::getInstance()->Decrypt($_REQUEST["contrato"]));
		$metodo_cobro = $_REQUEST["metodo_cobro"];
		$tipo_tarjeta=$_REQUEST['tipo_tarjeta'];
		$numero_tarjeta=$_REQUEST['numero_tarjeta'];
		$cvv_metodo=$_REQUEST['cvv_metodo'];
		$month_venc=$_REQUEST['month_venc'];
		$year_venc=$_REQUEST['year_venc'];
		$dias_debito=$_REQUEST['dias_debito']; 
		
		
		$con=new Contratos($protect->getDBLink());  
		$return=$con->guardar_tarjeta_de_credito($ct->serie_contrato,
												$ct->no_contrato,
												$tipo_tarjeta,
												$numero_tarjeta,
												$cvv_metodo,
		...
		$con->cambiar_metodo_cobro($ct->serie_contrato,$ct->no_contrato,$metodo_cobro);	
													
		echo json_encode($return); 
		exit;
	}
	echo json_encode(array("valid" =>false, "mensaje" =>"No se pudo procesar la informacion de forma correcta!"));
	exit;
}

if (isset($_REQUEST['doChangeMetodoCobro'])){  
	if (validateField($_REQUEST,"contrato") && validateField($_REQUEST,"fpago")){ 
	 	SystemHtml::getInstance()->includeClass("contratos","Contratos"); 	 
		$ct = json_decode(System::getInstance()->Decrypt($_REQUEST["contrato"]));
		$fpago = $_REQUEST["fpago"];
		
		$con=new Contratos($protect->getDBLink());  
		$return=$con->cambiar_metodo_cobro($ct->serie_contrato,$ct->no_contrato,$fpago);
		echo json_encode($return); 
		exit;
	}
	echo json_encode(array("valid" =>false, "mensaje" =>"No se pudo procesar la informacion de forma correcta!"));
...
if (isset($_REQUEST['doFiltrarRequerimientos'])){  
	include("dashboard/view/listado_req_cobros.php"); 
	exit;
}
		...
		$inf['OFICIAL']= utf8_encode($key);
			foreach($ase as $key =>$oficial){
				...
				foreach($oficial as $key =>$row){		
					...
					$inf['TOTAL_CLIENTES']=$row['total_cliente']; 					
...
if (isset($_REQUEST["exportMetaToPDF"])){
	...
	if (isset($_REQUEST['motorizado'])){
		$sp=explode(",",$_REQUEST['motorizado']);
		if (is_array($sp)>0){ 
			foreach($sp as $key =>$val){ 
				if ($val!=''){
					$valor=System::getInstance()->Decrypt($val);
	...
	include("print/cartera_cobro_pdf.php"); 
	exit;
}
...
if (isset($_REQUEST["exportMetaToExcel"])){
	...
	$xls= new ExportXLS("META_COBRO.xls");
	$xls->addHeader(array(
				"CATEGORIA",
				"NOMBRE_CLIENTE",
				"NOMBRE_OFICIAL",
	...
	$xls->sendFile();
	//exportToCsv("META_COBRO.csv",$n_cartera);
...
if (isset($_REQUEST['CGNotaDebito'])){  
	if (validateField($_REQUEST,"contrato")&& validateField($_REQUEST,"monto_abono")
		&& validateField($_REQUEST,"t_movimiento")){ 
			
		$contrato=json_decode(System::getInstance()->Decrypt($_REQUEST['contrato'])); 
		$t_mov=json_decode(System::getInstance()->Decrypt($_REQUEST['t_movimiento'])); 
		
		 

		if ((!$contrato->serie_contrato) && (!$t_mov->TIPO_DOC)){
			echo json_encode(array("mensaje"=>"Error recibo no generado","error"=>true));		
			exit;
		} 
		$T_DOCTO=$t_mov->TIPO_DOC; 
		$T_MOV=$T_DOCTO;
		...
		if($monto_abono>0){
			...
			$cdata=$con->getInfoContrato($contrato->serie_contrato,$contrato->no_contrato);
			$tasa=$cb->getTasaActual($cdata->tipo_moneda);	
			$monto_abono=$monto_abono/$tasa;

			$interes_a_pagar=$cdata->interes/$cdata->cuotas;
			$cuota_a_pagar_sin_interes=$cdata->valor_cuota-$interes_a_pagar;
			$valor_cuota=round($interes_a_pagar+$cuota_a_pagar_sin_interes,2);	

			$capital_a_pagar=($cuota_a_pagar_sin_interes/($cdata->valor_cuota+$d_concdataato->monto_penalizacion))*$monto_abono;
			$interes_a_pagar=($interes_a_pagar/($cdata->valor_cuota+$cdata->monto_penalizacion))*$monto_abono; 
			$valor_cuota=round($interes_a_pagar+$cuota_a_pagar_sin_interes,2);  
			$penalizacion=round(($cdata->monto_penalizacion/($cdata->valor_cuota+$cdata->monto_penalizacion))*$monto_abono,2); 
			
			if ($monto_abono>0){ 
				$trans=MVFactura::getInstance()->doCreateNOTADB($cdata->id_nit_cliente,
																$cdata->EM_ID,
																$cdata->no_contrato,
																$cdata->serie_contrato,
																$T_MOV,
																$T_DOCTO, //RECIBO CAJA VIRTUAL   
																$monto_abono, //MONTO
																$tasa, //tipo_cambio 
																$_REQUEST['comentario'],  //OBSERVACIONES
																"GENERANDO DOCUMENTO NOTA DE DEBITO" 
																);	
				...
				$SQL="SELECT * FROM `movimiento_factura` 
						WHERE
						 caja_no_docto='".$trans->NO_DOCTO."' 
						 AND caja_serie='".$trans->SERIE."' limit 1 ";
				...
				while($row=mysql_fetch_assoc($rs)){																	
					...
					if (isset($row['ID_MOV_FACT'])){
						$obj->ID_MOV_FACT=$row['ID_MOV_FACT'];
					}
					$obj->ID_NIT=$row['ID_NIT'];
					$obj->NO_DOCTO=$row['NO_DOCTO'];
					$obj->CAJA_SERIE=$row['SERIE'];		
					$obj->FECHA="curdate()";
					$obj->ID_CAJA=$row['ID_CAJA'];
					$obj->EM_ID=$row['EM_ID'];
					$obj->TIPO_DOC=$T_DOCTO;
					$obj->TIPO_MOV=$row['TIPO_MOV'];		
					$obj->MONTO_DOC=$row['MONTO']; 	
					$obj->CUOTA=$row['MONTO']; 
					$obj->TOTAL_MOV=$row['MONTO'];
					$obj->INTERESES_PAG=$interes_a_pagar;	
					$obj->CAPITAL_PAG=$capital_a_pagar;	
					$obj->NO_CUOTA=0; 
					$obj->TIPO_CAMBIO=$row['TIPO_CAMBIO'];
					$obj->IMPUESTO_PAG=0;
					$obj->MORA_PAG=0;
					$obj->MANTENIMIENTO=0;
					$obj->INICIAL=0; 
					$obj->OF_COBROS=$row['id_usuario'];
					$obj->MOTORIZADO=0;
					$obj->OBSERVACIONES="";								
