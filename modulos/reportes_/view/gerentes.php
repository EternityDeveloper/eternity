...
<?php 
/* 
$SQL="SELECT codigo_asesor,CONCAT(sys_personas.`primer_nombre`,' ',sys_personas.`segundo_nombre`,
	' ',sys_personas.`primer_apellido`,' ',sys_personas.segundo_apellido) AS nombre_asesor FROM sys_asesor
INNER JOIN sys_personas ON (sys_personas.id_nit=sys_asesor.id_nit)
WHERE sys_asesor.codigo_gerente_grupo='".$row->codigo_gerente."' and (SELECT COUNT(*) AS total FROM contratos
  WHERE  contratos.codigo_asesor=sys_asesor.codigo_asesor and 
  	contratos.fecha_venta BETWEEN '".$day_from."' and '".$day_to."'  AND contratos.estatus IN (1,13,20)  )>0 ";
*/
...
?>    
  ...
  onclick="toggle('ASE<?php echo $rsA['codigo_asesor'];?>');">
...
while($rowar= mysql_fetch_assoc($rsar)){ 
 ...
 ?>  
  <tr style="display:none;background:#AEFFAE" class="ASE<?php echo $rsA['codigo_asesor'];?>">
    <td height="25" colspan="1" style="padding-left:60px;"><?php echo $rowar['nombre_cliente']?>&nbsp;(<?php echo $rowar['contrato']?>)</td>
    <td colspan="4" align="center" style="background:#AEFFAE"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-hover">
      <tr>
        <td width="10%" height="25" align="center"><?php echo $rowar['estatus'];?></td>
