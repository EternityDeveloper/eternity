...
//http://localhost/sandpit/?mod_planillas/asesores/pl_bonopor_auxilio&noview&periodo=QXE0Wm11WmVzWXczUmhKVXdVaHNjdz09&type=dy81c0kzTzJnVXExTkMzaXlteGhWQT09
	...
	if($rowVerifica['bono'] == 0){
	    ...
	    /* Equipo de Asesores*/
	     $sql = "select a.codigo_asesor,
		               a.fecha_ingreso as fecha_ingreso_cia,
					   (TIMESTAMPDIFF(DAY,a.fecha_ingreso,CURDATE())/30) as antiguedad
			      from sys_asesor a,
				       sys_personas b			       
			     where a.id_nit = b.id_nit 
				   ...
				   and (TIMESTAMPDIFF(DAY,a.fecha_ingreso,CURDATE())/30)<=6
				   and a.codigo_gerente_grupo!=9
		...
		while($rowAsesores = mysql_fetch_array($rsAsesores)){ 
		  ...
		  // echo round($rowAsesores['antiguedad'],2);
		   	$sp=explode(".",round($rowAsesores['antiguedad'],2));
			$antiguedad=$rowAsesores['antiguedad']; 
		 	if ($sp[1]>0){ 
				$antiguedad=$sp[0]+1; 
			} 
		    ...
		    if(in_array($antiguedad, range(1,6))){
				...
				$rowMeses = mysql_fetch_assoc($rsMeses);
 ...
 FROM `producto_contrato` AS pc WHERE pc.id_estatus=1 AND 
  pc.serie_contrato=contratos.serie_contrato AND pc.no_contrato=contratos.no_contrato LIMIT 1)!='OSARIOS' ";
				...
				if($contratos['conteo'] >= $rowMeses['cumplimiento']){
				   $montoBonoAux = $rowMeses['bonoprimario'];					
				}
				if($contratos['conteo'] >= $rowMeses['ventacontrato']){ 
