<?php
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "DELETE") ){
     ...
     $query = "update cm_comisiones_tbl set estatus = 2 where tipo = 2 and orden = " . (int)$_REQUEST['orden']; 
	 $resultado = mysql_query($query);
  ...
  $sql    = "select * from cm_comisiones_tbl where tipo = 2 and estatus = 1 order by rangoini, rangofin";
...
function insertOpenDialog(){
   var accion = "INSERT";
   $.post("?mod_planillas/view/component/tbl_bonoplan",{"accion":accion},function(data){
	   var dialog=createNewDialog("Tabla Bono por Plan",data,430);
	   ...
	   $("#cancelar").click(function(){
		 ...
		 $("#procesar").click(function(){
			 $.post("?mod_planillas/view/pl_bonoplan_tbl",$("#frm_tbl_config").serializeArray(), function(data){
			 alert(data.mensaje);
...
function editOpenDialog(orden, accion){
   ...
   $.post("?mod_planillas/view/component/tbl_bonoplan",{"orden":orden, "accion":accion},function(data){
	   var dialog=createNewDialog("Tabla Bono por Plan",data,430);
