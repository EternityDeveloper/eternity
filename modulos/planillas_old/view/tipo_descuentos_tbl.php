<?php
  ...
  $titulo = "Conceptos Ingreso o Descuentos";
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "INSERT") ){
	 ...
	 if( $rowQuery['conteo'] == 0 ){
		 ...
		 $qrCorrel = "select ( MAX(idconcepto) + 1 ) as correl from tipo_descuentos_tbl ";
		 $rsCorrel = mysql_query($qrCorrel);
		 $rwCorrel = mysql_fetch_array($rsCorrel);
		 $idconcepto = $rwCorrel['correl'];
		 
         $query = "insert into tipo_descuentos_tbl ( idconcepto,
		                                         descripcion, 
		     									 tipo,
												 usuario,
												 fechau,
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "EDIT") ){
	 ...
	 if($rowQuery['conteo'] == 0){
		 $query = "Update tipo_descuentos_tbl
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "DELETE") ){
     ...
     $query = "update tipo_descuentos_tbl set estatus = 2 where idconcepto = " .(int)$_REQUEST['idconcepto']; 
	 $resultado = mysql_query($query);
  ...
  $sql    = "select * from tipo_descuentos_tbl where estatus = 1 order by idconcepto";
...
function insertOpenDialog(){
   var accion = "INSERT";
   $.post("?mod_planillas/view/component/tbl_descuentos",{"accion":accion},function(data){
	   ...
	   $("#cancelar").click(function(){
		 ...
		 $("#procesar").click(function(){
			 $.post("?mod_planillas/view/tipo_descuentos_tbl",$("#frm_tbl_config").serializeArray(), function(data){
			 alert(data.mensaje);
...
function editOpenDialog(orden, accion){
   ...
   $.post("?mod_planillas/view/component/tbl_descuentos",{"idconcepto":orden, "accion":accion},function(data){
	   var dialog=createNewDialog("Tabla Conceptos",data,430);
