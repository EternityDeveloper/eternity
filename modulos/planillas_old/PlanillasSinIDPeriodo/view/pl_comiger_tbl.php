<?php
  ...
  $titulo = "Tabla de Comisiones Gerentes";
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "INSERT") ){
	 ...
	 if($rowQuery['conteo'] == 0){
		  $query = "insert into cm_comision_gerente_tbl ( rangoini, 
												          rangofin, 
												          porcentaje,
														  usuario,
														  fechau,
														  estatus)
										         values (".(int)$_REQUEST['rangoini'].","
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "EDIT") ){
	 ...
	 if($rowQuery['conteo'] == 0){
		 $query = "Update cm_comision_gerente_tbl
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === "DELETE") ){
     ...
     $query = "update cm_comision_gerente_tbl set estatus = 2 where orden = " .(int)$_REQUEST['orden']; 
	 $resultado = mysql_query($query);
  ...
  $sql    = "select * from cm_comision_gerente_tbl where estatus = 1 order by rangoini, rangofin";
...
function insertOpenDialog(){
   var accion = "INSERT";
   $.post("?mod_planillas/view/component/tbl_comiger",{"accion":accion},function(data){
	   var dialog=createNewDialog("Tabla Gerentes",data,430);
	   ...
	   $("#cancelar").click(function(){
		 ...
		 $("#procesar").click(function(){
			 $.post("?mod_planillas/view/pl_comiger_tbl",$("#frm_tbl_config").serializeArray(), function(data){
			 alert(data.mensaje);
...
function editOpenDialog(orden, accion){
   ...
   $.post("?mod_planillas/view/component/tbl_comiger",{"orden":orden, "accion":accion},function(data){
	   var dialog=createNewDialog("Tabla Gerentes",data,430);
