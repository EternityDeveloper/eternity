 ...
 WHERE sys_parentesco.`id_nit`='".$idnit."' AND sys_parentesco.`id_nit_parentesco`='".$idnit_parentesco."' ";
	...
	*/
	...
	public function create_contrato(){
		...
		$contrato->porc_descuento=floatval($dmontos['sum_descuento_por']);
		...
		$contrato->dia_pago="DAY(STR_TO_DATE('".$_REQUEST['fecha_primer_pago']."','%d-%m-%Y'))";
...
//		$contrato->prospecto=$this->_prospecto['valid']==true?$this->_prospecto['data']['idnit']:0;
		...
		$SQL=$contrato->getSQL("insert","contratos"); 
	 	...
	 	$prueba = mysql_query($SQL);
                if (!$prueba) {
                      $this->message['error']=true;
                      $this->message['mensaje']="este es el error al crear contrato".mysql_error()."query".$SQL;
                      return false;
                      exit;
                 } 
 		...
 		foreach($this->_product_list as $key =>$producto){  
			...
			$producto->EM_ID=$contrato->EM_ID;
                        if ($producto->serv_codigo == null) {
                            $producto->serv_codigo = " ";
                        }
...
//			$this->registarVenta($contrato,$producto); 
			...
			$SQL=$producto->getSQL("insert","producto_contrato");
		 	...
		 	$prueba = mysql_query($SQL);
                        if (!$prueba) {
                            $this->message['error']=true;
                            $this->message['mensaje']="error producto contrato".mysql_error()."query".$SQL;
                            return false;
                            exit; 
                        }
	...
	private function actualiarVenta($serie_contrato,$no_contrato){
		...
		while($row=mysql_fetch_assoc($rs)){
			...
			$SQL="SELECT * FROM `cierres`  WHERE fecha_inicio_ventas<='".$row['fecha_venta']."'
					AND fecha_fin_ventas>='".$row['fecha_venta']."'  ";
			$rsX=mysql_query($SQL);
	...
	private function createMovContrato($serie_contrato,$no_contrato){
		...
		//	$mov_contrato->ID_CAJA=$ID_CAJA['ID_CAJA'];
			...
			$mov_contrato->NO_CUOTA=0;
			$mov_contrato->CUOTA=0;
			$mov_contrato->OF_COBROS=UserAccess::getInstance()->getID();
			$mov_contrato->MOTORIZADO=0;
			$mov_contrato->OBSERVACIONES='PRIMER MOVIMIENTO DEL CONTRATO';
				...
				if (validateField($prod,"serv_codigo")){ 
					for($i=0;$i<$prod->cantidad;$i++){
						...
						$obj->porc_enganche=$plan->por_enganche;
                                                ...
                                                if (is_null($plan->por_interes) or $plan->por_interes = ' ') {
                                                   $obj->porc_intereses=0;
                                                }
                                                else {
 ...
 FROM `producto_contrato` AS pc WHERE pc.id_estatus=1 AND 
  pc.serie_contrato=contratos.serie_contrato AND pc.no_contrato=contratos.no_contrato LIMIT 1) AS producto				  				
	...
	public function getCapitalInteresCuotaFromContrato($serie_contrato,$no_contrato,$anulado="N"){ 
		...
		$SQL="SELECT 	
					SUM(CL.interes_pagado) AS interes_pagado,
					SUM(CL.monto_pagado) AS monto_pagado,
					SUM(CL.monto_abono) AS monto_abono, 
					SUM(CL.DEBITO_CAPITAL) AS DEBITO_CAPITAL,
					SUM(CL.DEBITO_INTERES) AS DEBITO_INTERES,	
					SUM(CL.CREDITO_CAPITAL) AS CREDITO_CAPITAL,
					SUM(CL.CREDITO_INTERES) AS CREDITO_INTERES,
					...
					AND mv.`NO_CONTRATO`=CL.`NO_CONTRATO` AND
					 ...
					 mv.ANULADO='".$anulado."'  ".$sqlmv."  ) AS capital_pagado 
				...
				SELECT 
					...
					SUM(CTL.DEBITO_CAPITAL) AS DEBITO_CAPITAL,
					SUM(CTL.DEBITO_INTERES) AS DEBITO_INTERES,	
					SUM(CTL.CREDITO_CAPITAL) AS CREDITO_CAPITAL,
					SUM(CTL.CREDITO_INTERES) AS CREDITO_INTERES,	
				 ...
				 SELECT
					...
					IF (movimiento_contrato.TIPO_MOV='ND',
										(movimiento_contrato.CAPITAL_PAG),0) AS DEBITO_CAPITAL,
					IF ((movimiento_contrato.ID_CAMBIOS_FINANCIEROS=0) AND (movimiento_contrato.TIPO_MOV='ND'),
										(movimiento_contrato.intereses_pag),0) AS DEBITO_INTERES, 					
					IF (movimiento_contrato.TIPO_MOV='NC',
										(movimiento_contrato.CAPITAL_PAG),0) AS CREDITO_CAPITAL, 
					IF ((movimiento_contrato.ID_CAMBIOS_FINANCIEROS=0) AND (movimiento_contrato.TIPO_MOV='NC'),
										(movimiento_contrato.intereses_pag),0) AS CREDITO_INTERES,
				...
				movimiento_caja.ANULADO='".$anulado."'
		...
		while($row=mysql_fetch_object($rs)){   
			...
			$row->capital_cancelado=($row->capital_pagado+$cn->monto_capitalizado)+($row->DEBITO_CAPITAL-$row->CREDITO_CAPITAL);  
			
			$row->capital_pendiente=round(((($cn->precio_lista-$cn->descuento))-($row->capital_cancelado)),2);  
			$row->interes_pendiente=($interes-$row->interes_pagado)+($row->CREDITO_INTERES-$row->DEBITO_INTERES); 
	...
	public function activar(){
		...
		if ((validateField($this->data,"serie_contrato") 
			...
			$forma_pago=System::getInstance()->Decrypt($this->data['forma_pago']); 
                        $fecha_venta=$_REQUEST['fecha_venta2'];
...
//		$nuevo_precio_neto=(((($cdata->precio_lista-$cdata->descuento)-$cdata->enganche)-$capital_interes->capital_pagado))-$monto_abono;	  
	...
	//		}
		...
		//$interes=((($monto_abono*$por_financiamiento)/100)*($cdata->cuotas/12)/$cdata->cuotas);
		
  
	 // 	print_r($cdata->cuotas);
	//	exit;		  
