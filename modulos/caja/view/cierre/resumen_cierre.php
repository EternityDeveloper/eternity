...
?><table border="1"  class="table table-bordered table-striped table-hover" style="width:600px;">
 ...
 $data=$cierre->listarDesgloseCierreFormaPago($filter);
$total_monto=0;
	foreach($data as $key =>$row){ 
		...
		foreach($row as $keys =>$data){
			...
			$total_monto=$total_monto+($row['MONTO']);
			$id=System::getInstance()->Encrypt(json_encode(array("data"=>$data,"filter"=>$filter)));
		?>
  <tr>
    ...
    <td align="center"><button type="button" class="orangeButton _btcerrar" value="<?php echo $id;?>">Cerrar</button></td>
  </tr>
  <?php } ?>
  <tr>
    <td></td>
