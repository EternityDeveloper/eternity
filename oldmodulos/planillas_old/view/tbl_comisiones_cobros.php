<?php
  ...
  $titulo  = "Tabla de Comisiones Cobros";
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === 'INSERT') ){
	...
	$verificar = "select count(1) as conteo
	 ...
	 if($inserta['conteo']==0 ){ 
	  ...
	  $query = "insert into tbl_comisiones_cobros (tipo, 
												  rangoini, 
												  rangofin, 
												  porcentaje,
												  usuario,
  ...
  if ( (isset($_REQUEST['accion']) && $_REQUEST['accion'] === 'EDIT') ){
	 ...
	 if( $resultado['conteo'] > 0 ){ 
		...
		$retur['error']   = true; 
		 ...
		 $query = "Update tbl_comisiones_cobros
  ...
  $sql    = "select * from tbl_comisiones_cobros where tipo = 2 and estatus = 1 order by rangoini, rangofin";
...
function insertOpenDialog(){
 ...
 var accion = "INSERT";
 $.post("?mod_planillas/view/component/tbl_cobradores", {"accion":accion}, function(data){
	 
	 var dialog = createNewDialog("Tabla Cobradores",data,430);
	 var n = $('#'+dialog);
	 ...
	 $("#procesar").click(function(){
		 $.post("?mod_planillas/view/tbl_comisiones_cobros",$("#frm_tbl_config").serializeArray(), function(data){
         alert(data.mensaje);
	 	 $("#"+dialog).remove();
...
function editOpenDialog(orden, accion){
   ...
   $.post("?mod_planillas/view/component/tbl_cobradores",{"orden":orden, "accion":accion},function(data){
	   var dialog=createNewDialog("Tabla Cobradores",data,430);
       ...
       <?php 
		...
		?>		  
	           <tr>
                  ...
                  <td align="right"><?=number_format($row['rangoini'],2,".",",")?></td>
                  <td align="right"><?=number_format($row['rangofin'],2,".",",")?></td>
