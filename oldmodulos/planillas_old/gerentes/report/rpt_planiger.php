...
$sql = "select a.codigo_gerente,
				...
				SUM(IF(a.idconcepto = 6, a.monto,0)) as retenciones,
				SUM(IF(a.idconcepto = 7, a.monto,0)) as comi_pendiente,
				(SELECT SUM(`pl_planillas_gerentes`.comision) AS comision FROM 
					...
					AND anio='".(int)$anio."' AND mes='".(int)$mes. "'  
						AND codigo_gerente=a.codigo_gerente) as comision_pre
  ...
  <table width="90%" class="fsPage">
   ...
   <table width="90%" class="fsPage">
      <tr>
         ...
         <td width="77"  align="right">Comision Pendiente</td>
         <td width="77"  align="right">PreCierre</td>
         <td width="57"  align="right">Retenciones</td>
	  ...
	  $comi_pre=0;  	  
	    while($row = mysql_fetch_assoc($result)){
		  ...
		  $subtotal   = (((float)$row['comision'])+(float)$row['comi_pendiente'])-((float)$row['precierre']+(float)$row['anulados']+(float)$row['retenciones']);
		  
		  $comi_pre=$comi_pre+$row['comision_pre'];
		  $comision=$comision+$row['comision'];
		  $precierre=$precierre+$row['precierre'];
		  $anulados=$anulados+$row['anulados'];
		  $_subtotal=$_subtotal+$subtotal;
		  $comision_pendiente=$comision_pendiente+$row['comi_pendiente'];
	   ?>		
		  <tr>
             ...
             <td align="right"><?=number_format($row['comi_pendiente'],2,".",",")?></td>
             <td align="right"><?=number_format($row['comision_pre'],2,".",",")?></td>
	  ...
	  ?>
		  <tr>
		    ...
		    <td align="right"><?=number_format($comision_pendiente,2,".",",")?></td>
		    <td align="right"><?=number_format($precierre,2,".",",")?></td>
		    <td align="right">&nbsp;</td>
		    <td align="right"><?=number_format($anulados,2,".",",")?></td>
		    <td align="right"><?=number_format($_subtotal,2,".",",")?></td>
     </tr>      
   </table>
</page>
