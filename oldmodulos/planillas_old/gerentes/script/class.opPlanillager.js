var opPlanillager = new Class({
	
	dialog_container : null, 
	
	initialize : function(dialog_container, choice){
		this.main_class       = "opPlanillager";
	...
	doViewQuestion : function(){
		...
		/*instance.post("?mod_planillas/gerentes/view/pl_periodo_cierre",null, */
		instance.post("./?mod_planillas/pl_centralfile_ger&choice="+this.opc+"&view",null,
			...
			$("#procesar").click(function(){
				if ( $("#frm_datos_cierre").valid() ){
				  ...
				  /* window.location.href="./?mod_planillas/report/pl_reporte_planiase&mes=" + mes + 
				                        "&tipo_cierre="+$('input[name=tipo_cierre]:radio:checked').val()+
						   			    "&anio="+$("#anio").val();*/
				    window.open( "./?mod_planillas/gerentes/report/rpt_planiger&periodo=" + $("#periodo").val() + 
				                        "&type="+$('input[name=type]:radio:checked').val() + "&anio=" +$("#anio").val()  );
				}
				 /* redirect */
