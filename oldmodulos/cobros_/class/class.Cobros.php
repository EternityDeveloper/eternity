...
class Cobros{
	...
	public function cambioDeOficialContrato($contrato,$oficial,$motorizado){
		$valid=false;
		$SQL="SELECT COUNT(*) AS total FROM `cobros_contratos` WHERE  serie_contrato='".$contrato->serie_contrato."' AND no_contrato='".$contrato->no_contrato."' ";	
		$rs=mysql_query($SQL);	
...
WHERE cobros_contratos.nit_oficial='232323232323' ";
	...
	public function getDetalleCarteraCobrada($oficial){
		...
		$SQL.=" AND movimiento_caja.FECHA  BETWEEN DATE_FORMAT(CURDATE(),'%Y-%m-01')  AND LAST_DAY(CURDATE()) ) AS DT
		) AS FT ";
