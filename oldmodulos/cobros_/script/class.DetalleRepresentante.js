    var DetalleRepresentante = new Class({
	...
	doInit : function(id){ 
	   var instance=this;
	   this._contrato=id; 
      // instance.reloader(id);
	  
		instance.post("./?mod_cobros/delegate&view_representante",{
			"id":id
		 }, function(response){
			//$("#lbeneficiario").remove();
			$("#lrepresentante").html(response);
			
			$(".bcrepresentante").click(function(){
				var id = $(this).attr("id");
				//alert(id);
				var accion = $(this).val();
			   // alert(accion);
			  instance.editRepresentante(id, accion);
			});			   
	...
	editRepresentante : function(id, accion){
            var instance=this;	    
            this.doViewRepresentante(id);
            //var url      = window.location.href;
            this.removeListener("onCreateRepresentante");  
            this.addListener("onCreateRepresentante",function(data){  
                instance.post("./?mod_cobros/delegate&procesar_representante",{ 
                                "contrato":instance._contrato,
                                'representante':id,
                                'new_representante':(data),
                                'accion':accion
                },
                function(data){ 
                        alert(data.mensaje);
		...
		direccion.addListener("doLoadViewComplete",function(rsobj){
			...
			$("#bt_pros_select").click(function(){
				...
				if (($("#parentesco option:selected").val()!="")){		
					person_component.closeView();	
					//instance.fire("onCreateRepresentante",{"tipo":"mayor","data":data}); 	
	...
	processEditRepresentante: function(idnit,obj,id_beneficiario){
		var instance=this;  
		var person_component= new ModuloPersonas('Datos Personales',this.dialog_container);	
		var person= new Persona(person_component);
		//PONGO EL MODULO DE PERSONA EN MODO EDITAR
	...
	processCreateRepresentante : function(data,obj){
		var instance=this;
		var person_component= new ModuloPersonas('Datos Personales',this.dialog_container);
		
		var person= new Representante(person_component);
		...
		person.addListener("onCreateRepresentante",function(data){
	...
	draw_beneficiario : function(patern,bt_name,data){
        ...
        /*CREA LA VISTA PARA AGREGAR UN REPRESENTANTE*/
	doViewRepresentante: function(id){
		var instance=this; 
		
			var action={
				//Quiero agregarlo!
				doAdd : function(data){   
					instance.processCreateRepresentante(data,id);
				},
				...
				onClientExist : function(data){ 
					if ((true)){ 
						instance.processEditRepresentante(data.personal.id_nit,id,"");
					}else{
