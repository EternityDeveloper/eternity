    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <table width="100%" border="0"  style="margin:10px;">
          <tr>
            <td style="width:150px;"><strong>GERENTE</strong></td>
            <td><select name="_gerente" id="_gerente" style="width:300px">
...
' ',LTRIM(oficial.`primer_apellido`),' ',LTRIM(oficial.segundo_apellido)) AS nombre_asesor
					...
					INNER JOIN `sys_personas` AS oficial ON (`oficial`.id_nit=gg.id_nit )
					GROUP BY oficial.id_nit ";  
			$rs=mysql_query($SQL); 
			while($row=mysql_fetch_assoc($rs)){	
					?>
              <option  value="<?php echo System::getInstance()->Encrypt($row['codigo_gerente']); ?>"><?php echo $row['nombre_asesor'];?></option>
              <?php } ?>
            </select></td>
          </tr>
          <tr>
