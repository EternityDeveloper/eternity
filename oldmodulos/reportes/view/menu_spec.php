...
if (isset($_REQUEST['estatus'])){
	if ($_REQUEST['estatus']!=""){
		$id=System::getInstance()->Decrypt($_REQUEST['estatus']);
	}
}
//$id=isset($_REQUEST['estatus'])?System::getInstance()->Decrypt($_REQUEST['estatus']):'';
...
?>
  ...
  <tr>
    <td height="30" colspan="2" align="center"><table width="400" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><strong>ESTATUS</strong></td>
        <td><select name="estatus" id="estatus" class="form-control">
          <option value="">Seleccione</option>
          <?php 
		  $SQL="SELECT * FROM `sys_status` where id_status in (1,13,26,48,25,20)";
		  $rs=mysql_query($SQL);
		  while($row= mysql_fetch_assoc($rs)){?>
          <option value="<?php echo System::getInstance()->Encrypt($row['id_status']);?>" <?php echo $row['id_status']==$id?' selected ':''?>><?php echo $row['descripcion'];?></option>
          <?php } ?>
        </select></td>
      </tr>
    </table></td>
