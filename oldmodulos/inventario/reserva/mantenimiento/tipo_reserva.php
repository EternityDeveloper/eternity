...
if (isset($_REQUEST['submit_tipo_reserva'])){
	...
	SystemHtml::getInstance()->addTagScriptByModule("Class.reservaMantenimiento.js");
...
$(function(){
	...
	mantenimiento= new MantenimientoReserva('content_dialog','role_list');
...
<div class="fsPage">
  <table width="500" border="0">
    <tr>
      <td width="150%"><h2>Tipos de reservas </h2></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><table border="0" class="display" id="role_list" style="font-size:13px">
        ...
        <tbody>
          <?php
$SQL="SELECT * FROM tipos_reservas 
		INNER JOIN sys_status ON (tipos_reservas.estatus=sys_status.id_status) ";
$rs=mysql_query($SQL);
while($row=mysql_fetch_assoc($rs)){
	$encriptID=System::getInstance()->getEncrypt()->encrypt(json_encode($row),$protect->getSessionID());
?>
          <tr>
            <td height="25"><?php echo $row['id_reserva']?></td>
            <td align="center" ><?php echo $row['reserva_descrip']?></td>
            <td align="center" ><?php echo $row['abono']?></td>
