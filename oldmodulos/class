...
class PersonalData{
	...
	public function create(){
		...
		if (!$this->existClient($this->filterNit($this->data['numero_documento']))){
			...
			if ($obj->id_clasificacion = ' ') {
                             $obj->id_clasificacion = 0;
                        }
			...
			$rs=@mysql_query($SQL);
		        ...
		        if (!$rs) {
                             $this->message['mensaje']='error'.mysql_error().'query'.$SQL;
                             $this->message['error']=true;
                             return false;
                          }	
...
(SELECT
	...
	public function getTotalTelefonos($id_nit){
		...
		$rs=mysql_query($SQL);
                   ...
                   $this->message['error']=true;
                   $this->message['mensaje']='error query'.mysql_error().'query'.$SQL;
                   return false;
