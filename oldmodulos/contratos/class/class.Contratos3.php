  ...
  pc.serie_contrato=contratos.serie_contrato AND pc.no_contrato=contratos.no_contrato LIMIT 1) AS producto				  				
	...
	public function getCapitalInteresCuotaFromContrato($serie_contrato,$no_contrato){ 
		...
		$SQL="SELECT  
					...
					(SELECT SUM(mv.MONTO) AS monto_pagado
					FROM `movimiento_caja` AS mv 
					INNER JOIN `movimiento_factura` AS mf ON (mf.`CAJA_SERIE`=mv.`SERIE` AND mf.`CAJA_NO_DOCTO`=mv.`NO_DOCTO`)
					WHERE mf.TIPO_MOV IN ('INI','RES','CTI') 
					AND (mv.SERIE_DOC_ANUL IS NULL AND mv.NO_DOC_ANUL IS NULL)
					AND mv.`NO_CONTRATO`=movimiento_caja.`NO_CONTRATO` AND
					 mv.`SERIE_CONTRATO`=movimiento_caja.`SERIE_CONTRATO` AND
					 mv.ANULADO='N' )AS INICIAL,
					 
					 (SELECT SUM(mf.capital_pag+mf.TOT_ABONOS) AS capital_pagado
					FROM `movimiento_caja` AS mv 
					INNER JOIN `movimiento_contrato` AS mf ON (mf.`CAJA_SERIE`=mv.`SERIE` AND 
mf.`NO_DOCTO`=mv.`NO_DOCTO`)
					INNER JOIN `movimiento_factura` AS mff ON (mff.`CAJA_SERIE`=mv.`SERIE` AND 
						mff.`CAJA_NO_DOCTO`=mv.`NO_DOCTO`)
					WHERE mf.TIPO_MOV IN ('CAPITAL','ABO','CUOTA','CT')  
					AND (mv.SERIE_DOC_ANUL IS NULL AND mv.NO_DOC_ANUL IS NULL)
					AND mff.`NO_CONTRATO`=movimiento_caja.`NO_CONTRATO` AND
					 mff.`SERIE_CONTRATO`=movimiento_caja.`SERIE_CONTRATO` AND
			...
			AND movimiento_caja.`NO_CONTRATO`='".$no_contrato."' AND
			 ...
			 AND movimiento_contrato.TIPO_MOV IN ('CUOTA') ".$sqlmc;
			  
		 //  AND movimiento_caja.ID_CAMBIOS_FINANCIEROS=0 
		 //AND movimiento_contrato.TIPO_MOV IN ('ABO','CUOTA','CAPITAL')
			...
			movimiento_caja.`SERIE_CONTRATO`='".$serie_contrato."'  AND
			movimiento_caja.ANULADO='N'
			AND movimiento_contrato.TIPO_MOV IN ('ABO','CUOTA','CAPITAL') ".$sqlmc;
...
WHERE  producto_contrato.`serie_contrato`='". mysql_real_escape_string($serie_contrato)."' AND producto_contrato.`no_contrato`='". mysql_real_escape_string($no_contrato)."' ";
