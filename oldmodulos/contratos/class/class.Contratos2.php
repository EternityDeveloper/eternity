		...
		if (isset($_REQUEST['sSearch'])){
		  if (trim($_REQUEST['sSearch'])!=""){
			...
			$SQL="SELECT contratos.*,
				...
				WHERE contratos.`estatus` in ('1','40','54') ";
		 	$SQL.=$QUERY;
...
//		$contrato->prospecto=$this->_prospecto['valid']==true?$this->_prospecto['data']['idnit']:0;
		...
		//$mov_contrato=$this->createMovContrato($contrato);		 
...
//			$row->capital_pagado=round($row->capital_pagado+$row->INICIAL,2);
		...
		//	print_r($row);
//			$row->capital_cancelado=($row->capital_pagado+$cn->monto_capitalizado)+$row->INICIAL; 
//			$row->capital_pendiente=round(((($cn->precio_lista-$cn->descuento)-$row->INICIAL)-($row->capital_pagado+$row->monto_abono)),2);
