var PlanProductos = new Class({
	...
	_id_nit : 0,
	info_inicial : null,
	/*Esta opcion almacena la forma de financiamiento elegida */
	_sl_financiamiento : null,
	initialize : function(dialog_container){
		...
		this._contratoCaja = new ContratoCaja("mensaje_producto");
	...
	changeViewProduct : function(obj,serie_contrato,no_contrato){
		...
		instance.post("./?mod_contratos/listar",{"edit_producto":'1',"product":obj},function(data){ 
			...
			$("#bt_c_save_product").click(function(){
				//alert(instance._producto[instance._randViewID].data.product_id)
				var ifdata='<br><center><strong><p>Desea realizar el cambio de producto? </p> </strong></center><br><center><button type="button" id="caputra_si" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false"><span class="ui-button-text">SI</span></button>&nbsp;&nbsp;<button id="captura_no" type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false"><span class="ui-button-text">NO</span></button></center>';
					...
					$("#caputra_si").click(function(){ 
						...
						instance.post("./?mod_contratos/listar",{
							...
							"product":obj,
							"newproduct":instance._producto[instance._randViewID].data.product_id,
			...
			//	$('#'+instance.dialog_container).hideLoading();	
				...
				$("#bt_c_financiamiento_find").click(function(){ 
					...
					if (instance._producto[instance._randViewID].data!=null){
						finan.viewPlanListFromProduct(instance._producto[instance._randViewID].data.product_id,filtro);
				...
				$("#bt_c_add_product").click(function(){
					...
					if (valid){
						...
						instance.close(dialog);
						instance.fire("onPlanProductChange",data);
	...
	loadView : function(filtro){
		...
		instance.post("./?mod_contratos/listar",{
				...
				instance._contratoCaja.addListener("updateMonto",function(data){
					instance.monto_enganche=data.monto; 
					instance.info_inicial=data.transapciones; 
				});
				...
				$("#bt_c_add_product").prop('disabled',true);
				/*AGREGAR UN PRODUCTO*/
				$("#bt_c_add_producto").click(function(){
					...
					//	instance._contratoCaja.getMontoMonto(producto);
				...
				$("#bt_c_add_product").click(function(){
					var valid=true;
					var message="";
					var por_inicial=((totales.monto_enganche*100)/totales.precio_lista_menos_TotalDescuentoMonto);
					 
 					if ((por_inicial<10) || (por_inicial<=0)){
						 message="Error, el monto inicial debe de ser mayor o igual al 10% del capital a financiar!";
						 valid=false;	
					}
					...
					if (valid){
						...
						instance.close(dialog);
						instance.fire("onPlanProductSelect",data);
	...
	bind_totales : function(){
		...
		instance._totales_plan.addListener("onChangeData",function(){
			...
			/*SI EL PORCENTAJE ABONADO EN RESERVA ES MAYOR A UN 10% ENTONCES DEJAR
			PROCESAR LA PANTALLA*/
		/*	if (porcient_inicial>=10){
				$("#bt_c_add_product").prop('disabled',false);
				$("#alert_monto").hide();
			}else{
				$("#bt_c_add_product").prop('disabled',true);
				$("#alert_monto").show();
	...
	createCustomTableView: function(rand,totales){
		...
		var sp=producto.jardin.split("-");
		var fs=producto.fase.split("-");
		/*AGREGANDO EL TITULO*/
		$("#title_"+rand).html($.trim(sp[0])+"-"+$.trim(fs[0])+"-"+ pad($.trim(producto.bloque),3)+"-"+producto.lote);
