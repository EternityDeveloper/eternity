...
//echo $_SESSION['info_d']->id_comercial_gerente); 
$MES=array(
	"1"=>'ENE',
	"2"=>'FEB',
	"3"=>'MAR',
...
$(function(){ 			
	...
	$("#bt_regresar").click(function(){
		window.location.href="<?php echo $_SESSION['info_d']->back_url;?>";
	});
});
...
<div id="inventario_page" class="fsPage">
  <h2 style="color:#FFF;margin-top:0px;">Gestión de Ventas por Asesor</h2>
   <input type="submit" name="bt_regresar" id="bt_regresar" value="Regresar"  class="btn btn-warning bt-sm"  />
...
?>
          <td width="100" height="30" ><strong><a href=".?mod_prospectos/listar&report_ase_detalle&amp;type=<?php echo $_REQUEST['type'];?>&amp;p_fecha_desde=<?php echo $row['fecha_inicio_ventas'];?>&amp;p_fecha_hasta=<?php echo $row['fecha_fin_ventas'];?>"><?php echo $MES[$row['mes']].$row['ano'] ?></a></strong></td>
          <?php } ?>
          <td align="center">&nbsp;</td>
        </tr>
      </table></td>
    ...
    <tr>
      <td align="center">
      <form method="post" action="./?mod_prospectos/listar&report_ase_detalle" > 
    ...
    <tr>
      <td><strong><?php echo $_SESSION['info_d']->nombre;?></strong></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    </tr>
...
$SQL="SELECT 
 ...
 FROM 
 	`cache_listado_prospectos` AS reporte_cache_pilar
...
WHERE 	reporte_cache_pilar.id_status in (5,4,6,9) and 
	(reporte_cache_pilar.`codigo_gerente`='".$_SESSION['info_d']->id_comercial_gerente."' AND 
	  sys_asesor.codigo_asesor='".$_SESSION['info_d']->id_comercial."') AND 
...
$inf=array(
		"type"=>"TOTAL",
		"id_comercial_gerente"=>$_SESSION['info_d']->id_comercial_gerente,
		"id_comercial"=>$_SESSION['info_d']->id_comercial,
...
?>                      
                	    <tr>
                	      ...
                	      <td align="center" ><?php echo $row['A']+$row['B']+$row['C'];?></td>
                	      ...
                	      <td align="center" ><?php echo $row['CIERRE'];?></td>
                	      <td align="center"><?php echo $row['RESERVAS'];?></td>
              	      </tr>
<?php } ?></tbody>
<tfoot>
                	    <tr>
...
WHERE reporte_cache_pilar.id_status in (5,4,6,9) and 
(reporte_cache_pilar.codigo_gerente='".$_SESSION['info_d']->id_comercial_gerente ."' AND reporte_cache_pilar.codigo_asesor='".$_SESSION['info_d']->id_comercial ."') and  
	DATE_FORMAT(fecha_inicio, '%Y-%m-%d')  BETWEEN '". mysql_escape_string($fecha_desde) ."' AND '". mysql_escape_string($fecha_hasta) ."'  
...
?>
				...
				<tfoot>
                	    ...
                	    <tr>
						  ...
						  $inf['id_comercial_gerente']=$_SESSION['info_d']->id_comercial_gerente ;
						  $inf['id_comercial']=$_SESSION['info_d']->id_comercial;
