...
?>
          <td width="100" height="30" ><strong><a href=".?mod_prospectos/listar&report_gerente&amp;type=<?php echo $_REQUEST['type'];?>&amp;p_fecha_desde=<?php echo $row['fecha_inicio_ventas'];?>&amp;p_fecha_hasta=<?php echo $row['fecha_fin_ventas'];?>"><?php echo $MES[$row['mes']].$row['ano'];?></a></strong></td>
<?php } ?>
          <td align="center">&nbsp;</td>
        </tr>
      </table></td>
...
DATE_FORMAT(fecha_inicio, '%Y-%m-%d') BETWEEN '". mysql_escape_string($fecha_desde) ."' AND '". mysql_escape_string($fecha_hasta) ."' and  sys_gerentes_grupos.status=1  AND reporte_cache_pilar.descrip_actividad!='PROCESO CREACION AUDITORIA'
...
while($row= mysql_fetch_assoc($rs)){
	...
	$inf=array(
			"type"=>"TOTAL",
			"back_urls"=>"?mod_prospectos/listar&report_gerente&type=&p_fecha_desde=".$fecha_desde."&p_fecha_hasta=".$fecha_hasta,
