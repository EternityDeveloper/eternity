...
<div id="inventario_page" class="fsPage">
  <h2 style="color:#FFF;margin-top:0px;">Gestión de Ventas por gerente  </h2>
...
?>
          <td width="100" height="30" ><strong><a href=".?mod_prospectos/listar&report_gerente_ase&amp;type=<?php echo $_REQUEST['type'];?>&amp;p_fecha_desde=<?php echo $row['fecha_inicio_ventas'];?>&amp;p_fecha_hasta=<?php echo $row['fecha_fin_ventas'];?>"><?php echo $MES[$row['mes']].$row['ano'] ?></a></strong></td>
          <?php } ?>
          <td align="center">&nbsp;</td>
        </tr>
      </table></td>
...
WHERE 	reporte_cache_pilar.id_status in (5,4,6,9) and 
	(reporte_cache_pilar.`codigo_gerente`='".$_SESSION['info']->id_comercial_gerente."' )
...
while($row= mysql_fetch_assoc($rs)){
	...
	$inf=array(
			...
			"type"=>"TOTAL",
			"back_url"=>"?mod_prospectos/listar&report_gerente_ase&type=&p_fecha_desde=".$fecha_desde."&p_fecha_hasta=".$fecha_hasta,
...
$SQL="SELECT  
	...
	SUM(`CIERRE`) AS CIERRE,
	SUM(`RESERVAS`) AS RESERVAS,
	reporte_cache_pilar.codigo_asesor as id_comercial,
	reporte_cache_pilar.codigo_gerente as id_comercial_gerente
 FROM cache_listado_prospectos as `reporte_cache_pilar`
WHERE reporte_cache_pilar.id_status in (5,4,6,9) and  (reporte_cache_pilar.codigo_gerente='".$_SESSION['info']->id_comercial_gerente ."') and  DATE_FORMAT(fecha_inicio, '%Y-%m-%d')  BETWEEN '". mysql_escape_string($fecha_desde) ."' AND '". mysql_escape_string($fecha_hasta) ."'  
