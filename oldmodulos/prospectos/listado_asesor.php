...
if (!isset($protect)){
	...
	if (isset($_REQUEST['remove_asesor']) && isset($_REQUEST['id'])){
		SystemHtml::getInstance()->includeClass("estructurac","Asesores"); 
		$asesor=new Asesores($protect->getDBLink(),$_REQUEST);

		echo json_encode($asesor->remove($_REQUEST['id'],$_REQUEST['comentario']));	
		exit;
		
	}
	
	
	if (isset($_REQUEST['view_asesor'])){
		include("view/remove/detalle_asesor.php");
	...
	if (isset($_REQUEST['x_search'])){
		SystemHtml::getInstance()->includeClass("estructurac","Asesores"); 
		$asesor=new Asesores($protect->getDBLink(),$_REQUEST);
		echo json_encode($asesor->getMyAsesores($protect->getComercialID()));		
