...
if (!isset($protect)){
	...
	if (isset($_REQUEST['getPersonData'])){
		...
		if ($person->existClient($client_id)){
			$data['valid']=true;	
			$data['personal']=$person->getClientData($client_id);
		 
			$data['phone']=$person->getPhone($client_id);
			$data['email']=$person->getEmails($client_id);
			$data['address']=$person->getAddress($client_id);
			//print_r($data);
		}
			...
			while($row=mysql_fetch_assoc($rs)){	
				...
				array_push($data['aaData'],
					array( 
						...
						"email"=>$row['email']."",
						"option1"=>'<a href="./?mod_client/client_edit&id='.$encriptID.'"><img src="images/view.png" width="27" height="28" /></a>',
						"id_nit_en"=>$encriptID,
						"option2"=>'<a href="#" class="rs_add_link" id="'.$encriptID.'"><img src="images/netvibes.png" width="27" height="28" /></a>',			
					)
				);
...
$(document).ready(function(){
	...
	/*
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "./?mod_client/client_list",
		"sServerMethod": "POST",	
	*/
	gobal_table=$("#role_list").dataTable({
							...
							"bProcessing": true,
							"bServerSide": true,
							"sAjaxSource": "./?mod_client/client_list&dt_list=1",
							"sServerMethod": "POST",
							"aoColumns": [
