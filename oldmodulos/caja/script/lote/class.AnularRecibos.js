/*
	Modulo para eliminar AnularRECIBOS
*/
var AnularRecibos = new Class({
	...
	*/
	doView : function(){ 
		...
		this.post("./?mod_caja/delegate&lote_delegate&view_main",{ },
			...
			$("#title_template").html("Anular recibos");
			...
			$("#bt_caja_process").click(function(){
				if ($("#observacion").val()==""){
					alert('Debe de ingresar una observacion!');
					return false;	
				}
				$("#bt_caja_process").attr('disabled','disabled');
				instance.post("./?mod_caja/delegate&lote_delegate&procesar_anulado_recibos",
					{"observacion":$("#observacion").val(),"action":"remove"},function(data){ 
					 	alert(data.mensaje);
	...
	drawDetalleRecibo : function(data){
		...
		if (data.valid){ 	
			...
			$("#cantidad_total").html(data.cantidad_total);
