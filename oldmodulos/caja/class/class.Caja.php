	...
	*/
	...
	public function doFacturarRecibo($serie_recibo="",$no_recibo=""){ 
		...
		if (!validateField($this->_data,"recibo")){
			$data=array("error"=>true,"mensaje"=>'No ha seleccionado un recibo!1');
			return $data; 
			exit;	
		}
		$recibo=json_decode(System::getInstance()->Decrypt($this->_data['recibo']));
		if (!validateField($recibo,"SERIE")){
			$data=array("error"=>true,"mensaje"=>'No ha seleccionado un recibo!2');
			return $data; 
			exit;	
		}
		 
		if (!validateField($recibo,"NO_DOCTO")){
			$data=array("error"=>true,"mensaje"=>'No ha seleccionado un recibo!3');
			return $data; 
			exit;	
		}	
	...
	public function doFacturarPago(){ 
		...
		if (count($listado_recibos)<=0){
			if (!validateField($this->_data,'tipo_movimiento')){
				$data=array("error"=>true,"mensaje"=>'Debe de seleccionar un recibo!4');
				return $data; 
			}
			...
			$montos=$this->getMontoApagarYCobrar($this->_data['forma_pago_token']);
 			...
 			if (count($listado_recibos)<=0){	
				$this->session_restart();							 
				$data=array("error"=>true,"mensaje"=>'Debe de seleccionar un documento para facturar! es este nuevo+++');
				return $data; 
				exit;	
			} 
		}  
	...
	public function doImportRecibo($tipo_doc="RM",$serie_docto="",$no_docto=""){ 
		...
		if (!validateField($this->_data,'tipo_movimiento')){
			$data=array("error"=>true,"mensaje"=>'Debe de seleccionar un recibo!5');
			return $data; 
		}
 	...
 	//	$mov->OBSERVACIONES=$this->_data['observacion'];
                $mov->OBSERVACIONES='doImportRecibo'; 
	...
	public function doPreFacturar(){ 
		...
		if (!validateField($this->_data,"forma_pago_token")){
			$data=array("error"=>true,"mensaje"=>'No ha seleccionado una forma de pago! ***1');
			return $data; 
			exit;	
		}
 		$forma_pago=$this->getItem($this->_data[forma_pago_token]);
 		...
 		/*if (!is_array($forma_pago)){
			$data=array("error"=>true,"mensaje"=>'No ha seleccionado una forma de pago! ***2');
			return $data; 
			exit;
		} */	
		...
		if (!validateField($this->_data,'tipo_movimiento')){
			$data=array("error"=>true,"mensaje"=>'Debe de seleccionar un recibo!6');
			return $data; 
		}
		...
		if (!$inf['error']){ 
		  	$this->session_restart();	 
			$data=array("error"=>false,"mensaje"=>'Transacción realizada! es aqui donde esta malo '); 
			return $data; 
		}else{ 
			return $inf;	
		}
...
//			$tasa=$this->getTasaActual($d_contrato->tipo_moneda);		
		...
		$SQL=$obj->toSQL("insert"); 
  		mysql_query($SQL);  
                        $ktest= new ObjectSQL();
                        $ktest->texto='procesarMovimientoContrato '.$SQL;
                        $ktest->setTable('prueba');
                        $KSQL=$ktest->toSQL("insert");
                        mysql_query($KSQL); 
	...
	/*	if (isset($this->_data['isNCF'])){
		...
		$mov->NO_CONTRATO=$no_contrato;
		$mov->SERIE_CONTRATO=$serie_contrato;
		$mov->REP_VENTA=intval($this->_data['reporte_venta']);
		$mov->DESCUENTO=0;
		$mov->INICIAL="N"; 
		if ($tmovi->TIPO_MOV=="INI"){
			$mov->INICIAL="S"; 
 ...
 //		$mov->OBSERVACIONES=$this->_data['observacion'];
                $mov->OBSERVACIONES='doCrearReciboCaja'; 
		$mov->setTable('movimiento_caja'); 
	...
	*/
	public function doGeneraRecibo($tipo_doc="RM"){ 
		...
		if (!validateField($this->_data,'tipo_movimiento')){
			$data=array("error"=>true,"mensaje"=>'Debe de seleccionar un recibo!7');
			return $data; 
		}
		...
		foreach($forma_pago as $key =>$val){
			...
			$fpago->OLD_SERIE=$serie_doc;	
 			...
 			$SQL=$fpago->toSQL('insert');
                   ...
                   /*     $ktest= new ObjectSQL();
                        $ktest->texto='doGeneraRecibo '.$SQL;
                        $ktest->setTable('prueba');
                        $KSQL=$ktest("insert");
                        mysql_query($KSQL); */
                        ...
                        if (!resultado) {
                            die('consulta fallo'.mysql_error());
                        } 
 	 		// mysql_query($SQL);	
			$tipo_cambio=$val['tipo_cambio'];
			$MONTO_PAGO_CAJA=$MONTO_PAGO_CAJA+$fpago->MONTO;  
		...
		$mov->ID_RESERVA=$id_reserva; 
		$mov->NO_RESERVA=intval($no_reserva);		 
		$mov->MONTO=$MONTO_PAGO_CAJA;
		$mov->TIPO_MONEDA=$tipo_cambio>1?'DOLARES':'LOCAL';
		$mov->TIPO_CAMBIO=$tipo_cambio; 
		$mov->NO_CONTRATO=$no_contrato;
	...
	//	$mov->REP_VENTA=$this->_data['reporte_venta'];
		$mov->DESCUENTO=0;
		$mov->INICIAL="N";
                $mov->tipo_mov=$tmovi->TIPO_MOV; 
		if ($tmovi->TIPO_MOV=="INI"){
			$mov->INICIAL="S"; 
		} 	  		
 	//	$mov->OBSERVACIONES=$this->_data['observacion'];
                $mov->OBSERVACIONES='doGeneraRecibo';
