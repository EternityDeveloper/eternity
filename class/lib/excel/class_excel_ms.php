...
function mid_sqlparaexcel($sql, $arquivo){

$arquivo = trim($arquivo);

// define nome do arquivo
$this->nomeDoArquivoXls = $arquivo.date("His");

// Colsulta
	$db_link =mssql_connect("192.168.0.4","memorial","A123456a");
	mssql_select_db("SERVICIOSM",$db_link);
	$consReg = @mssql_query($sql,$db_link);
 
$linReg  = mssql_num_rows($consReg);

$linTable  = mssql_fetch_object($consReg);
...
mssql_data_seek($consReg,0);
$objects=mssql_fetch_object($consReg);
// Monta Colunas
//for($excel_coluna = 0; $excel_coluna < $qtdColunas; $excel_coluna++)

$consRegs = mssql_query($sql);
$excel_coluna=0;
$excel_linha=0;
foreach($objects as  $a => $d)
   {
    ...
    for($i=0; $i < $linReg; $i++)
	   {
       //pega registros
	   $valorCelula = mssql_result($consRegs, $i, $nome_coluna);
